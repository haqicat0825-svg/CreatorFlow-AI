...
import type { ContentTask, Draft, TitleCandidate } from "@/lib/types";

const fallbackBrief: ContentTask = {
  topic: "秋季韩系穿搭",
  audiences: ["18-25岁女生"],
  styles: ["韩系甜美"],
  goal: "种草",
...
export default function CreatorPage() {
  ...
  const [draft, setDraft] = useState<Draft>({ ...topics[0].draft });
  const [generated, setGenerated] = useState<GeneratedContent | null>(null);
  const [brief, setBrief] = useState<ContentTask>(fallbackBrief);
  const [generationState, setGenerationState] = useState<GenerationState>("idle");
  const [error, setError] = useState("");
  const [saved, setSaved] = useState(false);
  const [tab, setTab] = useState<"topics" | "copy" | "cover">("copy");
  const started = useRef(false);

  const generate = useCallback(async (task: ContentTask) => {
  ...
  useEffect(() => {
    ...
    let task = fallbackBrief;
    const stored = window.sessionStorage.getItem("creatorflow-task");
    if (stored) {
      try {
        task = JSON.parse(stored) as ContentTask;
      } catch {
        // Invalid legacy data is ignored; the safe default brief remains available.
      }
    }
    setBrief(task);
    void generate(task);
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-7 lg:px-10"><PageHeader eyebrow="AI-assisted editorial desk" title="Content Creator" description="从十个高匹配选题中挑选方向，再与你的 Writer Agent 一起完成内容。"/>
    ...
    <div className="mt-2 flex gap-2 rounded-2xl bg-white/60 p-1.5 lg:hidden">{[["topics","选题"],["copy","文案"],["cover","封面"]].map(([id,label])=><button key={id} onClick={()=>setTab(id as typeof tab)} className={`flex-1 rounded-xl py-2 text-sm ${tab===id?"bg-[var(--ink)] text-white":""}`}>{label}</button>)}</div>
    <div className="mt-6 grid min-h-[680px] gap-4 lg:grid-cols-[290px_minmax(360px,1fr)_320px]">
      ...
      <section className={`${tab!=="copy"?"hidden lg:block":""} panel flex flex-col p-5 sm:p-7`}><div className="flex items-center justify-between"><div><p className="fine">Writer Agent</p><h2 className="mt-1 text-xl font-bold">文案编辑器</h2></div><button disabled={generationState === "generating"} className="soft-button flex items-center gap-2 text-sm" onClick={()=>void generate(brief)}><RefreshCw className={generationState === "generating" ? "animate-spin" : ""} size={15}/>{generationState === "generating" ? "生成中…" : "重新生成"}</button></div><label className="mt-7 text-xs font-bold text-[var(--muted)]">标题<input aria-label="标题" value={draft.title} onChange={event=>setDraft({...draft,title:event.target.value})} className="mt-2 w-full border-b border-[var(--line)] bg-transparent pb-4 text-2xl font-bold outline-none"/></label><label className="mt-6 flex flex-1 flex-col text-xs font-bold text-[var(--muted)]">正文<textarea aria-label="正文" value={draft.body} onChange={event=>setDraft({...draft,body:event.target.value})} className="mt-2 min-h-[300px] flex-1 resize-none rounded-2xl bg-[var(--cream)] p-5 text-sm font-normal leading-7 text-[var(--ink)] outline-none"/></label><div className="mt-5 flex flex-wrap items-center justify-between gap-4"><div className="flex flex-wrap gap-2"><Hash size={15}/>{draft.tags.map(tag=><span key={tag} className="rounded-full bg-[var(--almond)] px-3 py-1 text-xs">{tag}</span>)}</div><span className="text-xs text-[var(--muted)]">{draft.body.length} 字</span></div></section>
      <aside className={`${tab!=="cover"?"hidden lg:flex":""} panel flex-col p-5`}><div className="flex items-center justify-between"><h2 className="font-bold">封面预览</h2><span className="fine">3:4</span></div><div className="visual-art mt-5 aspect-[3/4] rounded-[24px]" style={{"--art-a":draft.coverColors[0],"--art-b":draft.coverColors[1],"--art-c":draft.coverColors[2]} as React.CSSProperties}><div className="absolute inset-x-5 bottom-5 z-10 rounded-2xl bg-white/78 p-4 backdrop-blur-md"><p className="fine">Autumn look</p><p className="mt-1 font-bold leading-tight">{draft.title}</p></div></div>{generated?.coverPrompt && <><p className="mt-3 line-clamp-3 text-xs leading-5 text-[var(--muted)]">{generated.coverPrompt}</p><Link href="/visual-studio" onClick={openVisualStudio} className="soft-button mt-3 block w-full text-center text-sm">前往 Visual Studio 生成封面</Link></>}{generated?.ragReferences && generated.ragReferences.length > 0 && <div className="mt-4 border-t border-[var(--line)] pt-4"><p className="fine">引用来源 · {generated.ragReferences.length}</p><div className="mt-2 space-y-2">{generated.ragReferences.map(source => <div key={source.id} className="rounded-xl bg-[var(--cream)] px-3 py-2 text-xs"><p className="font-semibold">{source.title}</p>{source.sourceUrl && <a className="mt-1 block truncate text-[var(--rose-deep)]" href={source.sourceUrl} target="_blank" rel="noreferrer">查看原始来源</a>}</div>)}</div></div>}<button onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),1600)}} className="primary-button mt-5 flex items-center justify-center gap-2">{saved?<><Check size={16}/>已保存</>:<><Save size={16}/>保存草稿</>}</button></aside>
    </div>
  </main>;
}
