...
export function ModelConfigCard({ title, description, initial, options, modes }: { title: string; description: string; initial: ModelConfig; options: { value: ModelProvider; label: string }[]; modes?: boolean }) {
  ...
  return <section className="panel overflow-hidden">
    ...
    <div className="p-6">
      {modes && <div className="mb-6 flex gap-2">{options.map(o => <button key={o.value} aria-pressed={config.provider === o.value} onClick={() => setConfig({ ...config, provider: o.value, status: "disconnected" })} className={segmentedControlClass(config.provider === o.value)}>{o.label}</button>)}</div>}
